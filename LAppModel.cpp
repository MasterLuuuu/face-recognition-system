...
void LAppModel::LoadAssets(const csmChar* dir, const csmChar* fileName)
{
    ...
    ICubismModelSetting* setting = new CubismModelSettingJson(buffer, size); //模型设置信息
    DeleteBuffer(buffer, path.GetRawString());

    SetupModel(setting);

    CreateRenderer(); //释放渲染器
